use goxviet_core::infrastructure::engine::Engine;
...
fn main() {
    let word = std::env::args().nth(1).unwrap_or("is".to_string());
    ...
    eprintln!("Before space: '{}'", output);
    let result = engine.on_key(49, false, false); // SPACE
    eprintln!("Space result: action={} bs={} count={}", result.action, result.backspace, result.count);
    if result.action == 1 {
        for _ in 0..result.backspace.min(output.len() as u8) { output.pop(); }
        for i in 0..result.count as usize {
            unsafe { if let Some(c) = char::from_u32(*result.chars.offset(i as isize)) { output.push(c); } }
