...
fn test_viet_typing_sequence() {
    ...
    assert_eq!(
        r_e.action, 1,
        "e+e should trigger circumflex with mark repositioning"
    );
    ...
    assert!(
        output.contains('ế'),
        "Should output ế (ê with sắc mark), got: '{}'",
        output
    );
...
fn test_mark_repositioning_on_tone_change() {
    ...
    assert!(
        buffer.starts_with("iết"),
        "Expected 'iết', got '{}' - mark should be on ê",
        buffer
    );
}
...
fn test_viet_intermediate_states() {
    ...
    assert_eq!(
        r5.action, 1,
        "Step 5: e+e should trigger circumflex transformation"
    );
    assert!(
        r5.count > 0,
        "Step 5: should output ê with mark repositioned"
    );
    ...
    assert!(
        output5.contains('ế'),
        "Step 5 output should contain ế, got: '{}'",
        output5
    );
...
fn test_viese_sequence() {
    ...
    assert_eq!(
        r_e.action, 1,
        "e+e should trigger circumflex transformation (this was the bug!)"
    );
    ...
    assert!(
        output.contains('ế'),
        "Output should contain ế (ê with sắc), got: '{}'",
        output
    );
}
...
fn test_vieset_composition() {
    ...
    fn apply_to_composition(composition: &mut String, result: goxviet_core::engine::Result) {
        if result.action == 1 {
            ...
            if result.backspace > 0 {
                let chars_to_keep = composition
                    .chars()
                    .count()
                    .saturating_sub(result.backspace as usize);
    ...
    assert_eq!(
        r_e.action, 1,
        "e+e should trigger circumflex transformation"
    );
    apply_to_composition(&mut composition, r_e);
    // Composition should now have ê with mark
    assert!(
        composition.contains('ế'),
        "Composition should contain ế, got: '{}'",
        composition
    );
    ...
    assert!(
        composition.starts_with("viết") || composition.contains("viết"),
        "Final composition should contain 'viết', got: '{}'",
        composition
    );
}
...
fn test_uoi_with_horn() {
    ...
    assert!(
        buffer.starts_with("ưới"),
        "Expected 'ưới', got '{}' - mark should be on ơ (middle diacritic)",
        buffer
    );
}
