...
fn test_english_auto_restore_on_space() {
    ...
    println!("Test 'fix + space': action={}, backspace={}, count={}", 
    ...
    assert_eq!(result.action, 0, "Vietnamese 'mĩ' should not auto-restore on space");
...
fn test_bilingual_typing_with_auto_space() {
    ...
    println!("  → Action: {} (0=pass through, keeps Vietnamese)", r2.action);
...
#[test]
fn test_debug_tex_typing() {
    use goxviet_core::data::keys;
    use goxviet_core::engine::validation;
    use goxviet_core::engine::syllable::parse;
    
    let mut engine = Engine::new();
    engine.set_method(0); // Telex
    
    println!("\n=== Debug: typing 'text' character by character ===");
    
    // Type 't'
    let r1 = engine.on_key(keys::T, false, false);
    println!("After 't': action={}, backspace={}, count={}", r1.action, r1.backspace, r1.count);
    println!("  Buffer should be: [t]");
    
    // Type 'e'
    let r2 = engine.on_key(keys::E, false, false);
    println!("After 'e': action={}, backspace={}, count={}", r2.action, r2.backspace, r2.count);
    println!("  Buffer should be: [t, e]");
    
    // Type 'x' (this is tone modifier in Telex - should create tẽ)
    let r3 = engine.on_key(keys::X, false, false);
    println!("After 'x': action={}, backspace={}, count={}", r3.action, r3.backspace, r3.count);
    ...
    if r3.action == 1 {
        let output: String = (0..r3.count as usize)
            ...
            .collect();
        println!("  Output: {:?}", output);
        println!("  Expected: backspace=1 (delete 'e'), output='ẽ' (e with ngã tone)");
        println!("  Buffer after transform: [t, ẽ]");
    }
    
    // Now type second 't'
    let r4 = engine.on_key(keys::T, false, false);
    println!("After second 't': action={}, backspace={}, count={}", r4.action, r4.backspace, r4.count);
    println!("  Buffer should be: [t, ẽ, t]");
    
    // Check if pattern is detected as English
    let buffer_keys = vec![keys::T, keys::E, keys::T];
    let syllable = parse(&buffer_keys);
    println!("Syllable parse: initial.len()={}, vowel.len()={}, final.len()={}", 
             syllable.initial.len(), syllable.vowel.len(), syllable.final_c.len());
    
    // Test detection with 'X' as modifier (what was originally typed)
    let is_english_x = validation::is_foreign_word_pattern(&buffer_keys, keys::X);
    println!("Is 'tet' detected as English with X modifier? {}", is_english_x);
    
    // Test detection with 'T' as modifier (what handle_normal_letter sees)
    let is_english_t = validation::is_foreign_word_pattern(&buffer_keys, keys::T);
    println!("Is 'tet' detected as English with T modifier? {}", is_english_t);
    
    // Press space
    let r5 = engine.on_key(keys::SPACE, false, false);
    println!("After space: action={}, backspace={}, count={}", r5.action, r5.backspace, r5.count);
    ...
    if r5.action == 1 {
        ...
        println!("No auto-restore happened - is_english_word flag was not set");
    }
}
...
fn test_bug_backspace_after_tet_space() {
    ...
    println!("3. Type 's': action={}, backspace={}", r3.action, r3.backspace);
    if r3.action == 1 {
        let output: String = (0..r3.count as usize)
            .filter_map(|i| char::from_u32(r3.chars[i]))
            .collect();
    ...
    println!("6. Backspace #1: action={}, backspace={}, count={}", 
    ...
    println!("7. Backspace #2: action={}, backspace={}, count={}", 
    ...
    println!("10. Type 'x' (ngã tone): action={}, backspace={}, count={}", 
    ...
    if r10.action == 1 && r10.count > 0 {
        ...
        if final_result.contains("tt") {
            panic!("❌ BUG DETECTED: Got double 't' - {:?}. Expected 'tẽ' or 'ẽ'", final_result);
...
fn test_exact_bug_scenario_test_space_back_back_text() {
    ...
    println!("Type 'x': action={}, backspace={}, count={}", 
    ...
    if r_final_space.action == 1 {
        ...
        if final_output.starts_with("tt") {
            panic!("❌ BUG CONFIRMED: Got '{}' with double 't'! Expected 'tẽt '", final_output);
        ...
        println!("\n--- Final Result ---");
        println!("No auto-restore (action={}), buffer contains final word", r_final_space.action);
    }
