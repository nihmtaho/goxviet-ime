...
fn test_res_sac_tone() {
    ...
    println!(
        "After 's': action={}, backspace={}, count={}, buffer='{}'",
        r3.action,
        r3.backspace,
        r3.count,
        e.get_buffer()
    ...
    assert_eq!(
        r3.backspace, 1,
        "Should have backspace=1 to replace 'e' with 'é'"
    );
}
...
fn test_restore_english_word() {
    ...
    println!(
        "After SPACE: backspace={}, result='{:?}'",
        space_result.backspace,
    ...
    let restored_text: String = space_result.as_slice()[..space_result.count as usize]
        ...
        .collect();
    assert!(
        restored_text.contains("restore")
            || after_space.contains("restore")
...
fn test_disconnect_english_word() {
    ...
    assert!(
        !final_buffer.contains("diconnect"),
        "Should not have 'diconnect' substring, got '{}'",
        final_buffer
    );
}
