...
fn decompose_vn(c: char) -> (char, Option<char>, Option<char>) {
    match c {
        'à' => ('a', None, Some('f')), 'á' => ('a', None, Some('s')),
        'ả' => ('a', None, Some('r')), 'ã' => ('a', None, Some('x')),
        'ạ' => ('a', None, Some('j')),
        'è' => ('e', None, Some('f')), 'é' => ('e', None, Some('s')),
        'ẻ' => ('e', None, Some('r')), 'ẽ' => ('e', None, Some('x')),
        'ẹ' => ('e', None, Some('j')),
        'ì' => ('i', None, Some('f')), 'í' => ('i', None, Some('s')),
        'ỉ' => ('i', None, Some('r')), 'ĩ' => ('i', None, Some('x')),
        'ị' => ('i', None, Some('j')),
        'ò' => ('o', None, Some('f')), 'ó' => ('o', None, Some('s')),
        'ỏ' => ('o', None, Some('r')), 'õ' => ('o', None, Some('x')),
        'ọ' => ('o', None, Some('j')),
        'ù' => ('u', None, Some('f')), 'ú' => ('u', None, Some('s')),
        'ủ' => ('u', None, Some('r')), 'ũ' => ('u', None, Some('x')),
        'ụ' => ('u', None, Some('j')),
        'ỳ' => ('y', None, Some('f')), 'ý' => ('y', None, Some('s')),
        'ỷ' => ('y', None, Some('r')), 'ỹ' => ('y', None, Some('x')),
        'ỵ' => ('y', None, Some('j')),
        'â' => ('a', Some('a'), None), 'ầ' => ('a', Some('a'), Some('f')),
        'ấ' => ('a', Some('a'), Some('s')), 'ẩ' => ('a', Some('a'), Some('r')),
        'ẫ' => ('a', Some('a'), Some('x')), 'ậ' => ('a', Some('a'), Some('j')),
        'ă' => ('a', Some('b'), None), 'ằ' => ('a', Some('b'), Some('f')),
        'ắ' => ('a', Some('b'), Some('s')), 'ẳ' => ('a', Some('b'), Some('r')),
        'ẵ' => ('a', Some('b'), Some('x')), 'ặ' => ('a', Some('b'), Some('j')),
        'ê' => ('e', Some('e'), None), 'ề' => ('e', Some('e'), Some('f')),
        'ế' => ('e', Some('e'), Some('s')), 'ể' => ('e', Some('e'), Some('r')),
        'ễ' => ('e', Some('e'), Some('x')), 'ệ' => ('e', Some('e'), Some('j')),
        'ô' => ('o', Some('o'), None), 'ồ' => ('o', Some('o'), Some('f')),
        'ố' => ('o', Some('o'), Some('s')), 'ổ' => ('o', Some('o'), Some('r')),
        'ỗ' => ('o', Some('o'), Some('x')), 'ộ' => ('o', Some('o'), Some('j')),
        'ơ' => ('o', Some('h'), None), 'ờ' => ('o', Some('h'), Some('f')),
        'ớ' => ('o', Some('h'), Some('s')), 'ở' => ('o', Some('h'), Some('r')),
        'ỡ' => ('o', Some('h'), Some('x')), 'ợ' => ('o', Some('h'), Some('j')),
        'ư' => ('u', Some('h'), None), 'ừ' => ('u', Some('h'), Some('f')),
        'ứ' => ('u', Some('h'), Some('s')), 'ử' => ('u', Some('h'), Some('r')),
        'ữ' => ('u', Some('h'), Some('x')), 'ự' => ('u', Some('h'), Some('j')),
        'đ' => ('d', Some('d'), None),
        _ => (c, None, None),
    }
}
...
fn vn_to_telex_tone_early(word: &str) -> String {
    ...
    while i < chars.len() {
        ...
        let is_vowel = matches!(base_lower, 'a' | 'e' | 'i' | 'o' | 'u' | 'y')
            && mark != Some('d');
    ...
    while i < chars.len() {
        ...
        if i + 1 < chars.len() && (c == 'o' || c == 'u') {
            ...
            let (new_first, new_next) = match next {
                'à' if c == 'o' => ('ò', 'a'), 'á' if c == 'o' => ('ó', 'a'),
                'ả' if c == 'o' => ('ỏ', 'a'), 'ã' if c == 'o' => ('õ', 'a'),
                'ạ' if c == 'o' => ('ọ', 'a'),
                'è' if c == 'o' => ('ò', 'e'), 'é' if c == 'o' => ('ó', 'e'),
                'ẻ' if c == 'o' => ('ỏ', 'e'), 'ẽ' if c == 'o' => ('õ', 'e'),
                'ẹ' if c == 'o' => ('ọ', 'e'),
                'ỳ' if c == 'u' => ('ù', 'y'), 'ý' if c == 'u' => ('ú', 'y'),
                'ỷ' if c == 'u' => ('ủ', 'y'), 'ỹ' if c == 'u' => ('ũ', 'y'),
                'ỵ' if c == 'u' => ('ụ', 'y'),
...
fn words_match(expected: &str, actual: &str) -> bool {
    expected == actual
        || to_modern_tone_simple(expected) == to_modern_tone_simple(actual)
}
...
fn test_tone_early_from_69k() {
    let content = fs::read_to_string(
        "tests/data/vietnamese_69k_pure.txt",
    )
    ...
    for word_raw in &raw_words {
        ...
        if word.chars().any(|c| c.is_ascii_digit() || c == '-' || c == '\'' || c == '.') {
            continue;
        }
    ...
    if let Ok(mut f) = fs::File::create(failures_path) {
        let _ = writeln!(f, "# Tone-early Telex failures ({} / {} failed)", total - passed, total);
    ...
    let closed_rate = if closed_total > 0 { closed_passed as f64 / closed_total as f64 * 100.0 } else { 0.0 };
    let open_rate = if open_total > 0 { open_passed as f64 / open_total as f64 * 100.0 } else { 0.0 };
    ...
    println!("───────────────────────────────────────────────────────────");
    println!("  ALL WORDS:     {:>6} / {:>6}  ({:.2}%)", passed, total, rate);
    println!("  CLOSED (CVC):  {:>6} / {:>6}  ({:.2}%)", closed_passed, closed_total, closed_rate);
    println!("  OPEN   (CV):   {:>6} / {:>6}  ({:.2}%)", open_passed, open_total, open_rate);
    println!("  Failures: {} (written to {})", total - passed, failures_path);
    println!("───────────────────────────────────────────────────────────");
    if !failures.is_empty() {
        ...
        if failures.len() > 20 {
            println!("  ... and {} more (see {})", failures.len() - 20, failures_path);
        }
    }
    println!("═══════════════════════════════════════════════════════════\n");
    ...
    assert!(
        ...
        closed_rate, closed_total - closed_passed, closed_total, failures_path
    );
}
