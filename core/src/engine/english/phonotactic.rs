...
impl PhonotacticEngine {
    ...
    pub fn analyze(keys: &[(u16, bool)]) -> PhonotacticResult {
        ...
        let weights = [100, 98, 95, 90, 91, 75, 85, 80];
    ...
    /// L1: Check for invalid initials (F, J, W, Z at word start)
    ...
    fn check_double_consonants(keys: &[(u16, bool)]) -> u8 {
        const DOUBLE_CONSONANTS: &[u16] = &[
            keys::L, keys::S, keys::F, keys::R, keys::T, keys::D, keys::N, keys::M, keys::P,
            keys::C, keys::Z, keys::X, keys::V,
        ];
    ...
    fn check_suffixes(keys: &[(u16, bool)]) -> u8 {
        ...
        if keys.len() >= 3 {
            for suffix in SUFFIXES_3 {
                ...
                if &keys[start..start + 3].iter().map(|k| k.0).collect::<Vec<_>>()[..]
                    == &suffix[..]
                {
                    return 90;
                }
        ...
        if keys.len() >= 4 {
            for suffix in SUFFIXES_4 {
                ...
                if &keys[start..start + 4].iter().map(|k| k.0).collect::<Vec<_>>()[..]
                    == &suffix[..]
                {
                    return 90;
                }
    ...
    /// L6: Check for English prefixes (un-, re-, pre-, dis-, etc.)
...
mod tests {
    ...
    fn test_layer1_invalid_initials() {
        ...
        assert!(result.layer_scores[0] > 0, "Should detect F as invalid initial");
    }
    ...
    fn test_vietnamese_invalid_initials() {
        ...
        let result = VietnameseSyllableValidator::validate(&keys);
        assert!(!result.rules_passed[0], "FOR should fail rule 1 (invalid initial)");
    }
