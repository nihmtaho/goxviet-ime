...
impl Engine {
    ...
    pub fn on_key_ext(&mut self, key: u16, caps: bool, ctrl: bool, shift: bool) -> Result {
        ...
        let should_skip = if self.method == 0 {  // Telex mode
            ...
            let is_dict_word = crate::engine_v2::english::dictionary::Dictionary::is_english(&temp_keys);
        ...
        if !skip_modifiers {
            if let Some(mark_val) = m.mark(key) {
                if let Some(result) = self.try_mark(key, caps, mark_val) {
                    ...
                    // NOTE: Do NOT call check_and_restore_english here!
                    // If the user explicitly typed a tone mark (s, f, r, x, j in Telex),
                    // we should honor their intent and not auto-restore to English.
                    // The mark was intentionally applied by the user.
        ...
        if self.buf.len() > 0 {
            ...
            if last_key == keys::A && last_tone == tone::HORN && last_mark == mark::NONE {
                ...
                if let Some(c) = self.buf.get_mut(pos) {
                    ...
                    // Pop the 'w' key from raw_input since it acted as a modifier (revert)
                    self.raw_input.pop();
    ...
    fn handle_normal_letter(&mut self, key: u16, caps: bool, _shift: bool) -> Result {
        ...
        */
        ...
        //
        // CRITICAL FIX: Do NOT auto-restore if user has EXPLICITLY applied tone marks
        // (s, f, r, x, j in Telex = acute, grave, hook, tilde, dot below)
        // These marks indicate INTENTIONAL Vietnamese typing by the user.
        // Auto-restoring would erase the user's explicit intent.
        //
        // Example: typing "phat" could be English, BUT if user typed "pha" + "s" (acute mark),
        // they explicitly marked it as Vietnamese "phá". When they add "t" to get "phát",
        // we should NOT restore to English "phat" just because it looks like English.
        let has_explicit_marks = self.buf.iter().any(|c| c.mark > 0);
    ...
    fn check_and_restore_english(&mut self, offset: u8, strict_mode: bool) -> Option<Result> {
        ...
        let is_dict = crate::engine_v2::english::dictionary::Dictionary::is_english(&raw_key_list);
        ...
        if is_dict {
            ...
            return Some(self.instant_restore_english());
        }
