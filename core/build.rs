//! Build script: generate Vietnamese and English dictionary data at compile time.
//!
...
//! - `english_words.rs`: phf::Set<&'static str> for EnglishWords.txt (~11K words, O(1))
...
fn main() {
    ...
    let english_words_path = Path::new(&manifest_dir).join("data/EnglishWords.txt");
    ...
    println!("cargo:rerun-if-changed=data/EnglishWords.txt");

    generate_viet_syllables(&tu_dien_path, &out_dir);
    generate_viet_compound_bin(&tu_ghep_path, &out_dir);
    generate_english_words(&english_words_path, &out_dir);
}
...
/// Generate phf::Set<&'static str> for common English words.
/// Output: OUT_DIR/english_words.rs
fn generate_english_words(path: &Path, out_dir: &str) {
    let file = File::open(path).expect("Cannot open EnglishWords.txt");
    let reader = BufReader::new(file);

    let mut entries: Vec<String> = Vec::new();
    for line in reader.lines() {
        let line = line.expect("Failed to read EnglishWords.txt line");
    ...
    let out_path = Path::new(out_dir).join("english_words.rs");
    let mut out = File::create(&out_path).expect("Cannot create english_words.rs");
    ...
    write!(
        out,
        "/// English word dictionary (~{} entries).
/// Generated at build time from data/EnglishWords.txt.
/// O(1) lookup via perfect hash function.
static ENGLISH_WORDS: phf::Set<&'static str> = {};",
        entries.len(),
        set_builder.build()
    )
    .expect("Failed to write english_words.rs");

    eprintln!(
        "cargo:warning=Generated ENGLISH_WORDS with {} entries",
        entries.len()
    );
}
