...
class PerAppModeManager {
    ...
    // App-specific IME state (enabled/disabled per app)
    ...
    func handleAppSwitch(_ bundleId: String) {
        guard bundleId != currentBundleId else { return }
        
        currentBundleId = bundleId
        Log.info("App switched to: \(bundleId)")
        
        // Restore IME state for this app
        if let enabled = appStates[bundleId] {
            NotificationCenter.default.post(name: .toggleVietnamese, object: enabled)
        }
    }
    
    func setStateForCurrentApp(_ enabled: Bool) {
        guard let bundleId = currentBundleId else { return }
        appStates[bundleId] = enabled
        Log.info("Set IME state for \(bundleId): \(enabled)")
    }
}
